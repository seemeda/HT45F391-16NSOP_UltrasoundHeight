...
IF 0 ;;TEST+
	MOV	A,00001000B		;;SUM > Comparator Value 
	MOV	ENVCMP,A
	CLR 	F_MY_TEST
	...
	CLR		T0PAU   ;;	运行TM0计数器
	SET 	ENVCMPE
MY_SAMPLEING:;;退出此 L_SAMPLING 循环条件：1.超时20ms  2.ENVCMPF 自动包络比较器中断
	SZ		__F_TIMEUP
...
$0:
   ; CLR     __R_STOP_CNT         
    CLR     __R_START_CNT 
    CLR     __R_TEMP6
	MOVF	__R_TEMP0,0    ;;计数大于阀值的个数  IAR0 >= __R_ADCOMPARE_VALUE，__R_TEMP0加1
	MOVF	__R_TEMP2,10   ;;__R_TEMP2  每比较一个FIFOOUT时__R_TEMP2减1，计数10个退出
	MOVF	MP0,OFFSET __R_DATA	
L_0:
    INC     __R_DET_CNT 	;;每次大循环都累计计数
    INC     __R_START_CNT  ;;每次加1，    __R_STOP_CNT = 10 - __R_START_CNT
    MOVF	__R_TEMP6,IAR0
    MOV     A,__R_MAX_DATA
    SUB     A,__R_TEMP6
    SZ      C 
    JMP     $0;L_1  ;;__R_MAX_DATA >= IAR0
    ...
    SUB     A,4;4 
    SNZ     C
    JMP     $1 ;;__R_DET_CNT - __R_MAXDET_CNT < 4
	
	;;__R_DET_CNT - __R_MAXDET_CNT >= 4
...
$1:    ;;__R_DET_CNT - __R_MAXDET_CNT < 4
    MOVF    __R_MAX_DATA, IAR0
    MOV     A,1
	ADDM    A,__R_MAX_DATA
	SZ      C
...
L_1:    
	...
	SNZ		C
	JMP		L_3 ;;IAR0 < __R_ADCOMPARE_VALUE
	JMP		L_2 ;;IAR0 >= __R_ADCOMPARE_VALUE
L_2: ;;IAR0 >= __R_ADCOMPARE_VALUE
	INC		__R_TEMP0 
	JMP		L_3
L_3:   ;;IAR0 < __R_ADCOMPARE_VALUE
	INC		MP0
	SDZ		__R_TEMP2
	JMP		L_0
	MOV		A,__R_TEMP0
...
;	CLR		__R_TIME_M
    SNZ     __F_GET_DISTANCE
	JMP		L_SAMPLING		
L_GET_DISTANCE:  ;;IAR0 >= __R_ADCOMPARE_VALUE 的数量个数 大于等于5次
    SNZ    __F_GET_DISTANCE
	JMP    $0
	JMP    $1
$0:	
