...
L_STOP_DISTANCE:
    CLR	 FSTART
   	CLR		PB2 ;;TEST+
   	;;功能：求出回波幅度最高点的时间，再减去固定最高点离回波刚到达时的周期数是不变的，得出回波到达点的时间
   	;;目前这里乘除没用到
   	;;这里乘以125再除以100
   	;;理解：125为40KHz回波半周期的12.5us时间，除以100是因为计数变量最小以10us为精度的
   	;;
   	;;
    ...
    ;;--------------TEST+ 
    IF 1 ;;TEST+
	;;幅度最大点时间减去离回波刚到达的固定周期数(初步计数是12个周期300us)的时间，得到回波刚刚到达的时间.
	MOV     A,__R_TIME_H
	SUB     A,3  
	MOV     __R_TIME_H,A
	;;减去2*9us
